...
select count(*) AS TotalOrders, sum(case when ship_date = order_date then 1 else 0 end) as SameShippedOrrders,
...
### the 'limit' keyword is not recognized in MS SQL.
### instead, we should ust the 'top' kwywaord to limit the number of rows returned by the query.
select top 3 customer_name, round(sum(sales), 2) as total_sales
from [dbo].[superstore]
...
select top 5 product_name,  round(avg( sales),2) as average_sales
from[dbo].[superstore]
